...
internal static class LeagueClientTask
{
    ...
    internal static void LolClientTask()
    {
        if (LeagueClientIsOpen())
        {
            ...
            if (Global.Region == "")
            {
                ...
                var response = Requests.WaitSuccessClientRequest("GET", "/riotclient/region-locale", true)[1];
    ...
    private static void LoadSummonerId()
    {
        ...
        var currentSummoner = Requests.WaitSuccessClientRequest("GET", "lol-summoner/v1/current-summoner", true);
